#!/usr/bin/env bash
# reproduce_spark_tests.sh — сборка и запуск юнит-тестов ГПСЧ и Думитреску из публичного
# дерева stoch/. Ожидается: RNG_TEST: ALL PASS; DUMI_TEST: ALL PASS.
# Требования: CUDA >= 12 (nvcc), GPU >= sm_70.
set -euo pipefail
cd "$(dirname "$0")/.."
