// tokenizer_json_test.cpp - host-only smoke test for the native
// tokenizer.json loader. Builds a tiny synthetic byte-level BPE fixture in
// /tmp and checks encode/decode/special/eos. No engine, no GPU, no model.
...
int main() {
    // ids: h=0 e=1 l=2 o=3 ' '=4 w=5 r=6 d=7 0xC3=8 0xA9=9, merges -> 10..18,
    ...
    CHECK(tok->encode("\xC3\xA9", ids));  // 'é' through single-byte tokens
    CHECK(ids.size() == 2 && ids[0] == 8 && ids[1] == 9);
    CHECK(tok->decode_token(ids[0]) + tok->decode_token(ids[1]) == "\xC3\xA9");
    CHECK(tok->eos() == 901);
