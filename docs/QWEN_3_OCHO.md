...
- Temperature-1.0 full-vocab Gumbel sampling contaminates long horizons; a candidate-set
  (top-p/top-k) restricted sampler or a lower temperature is the recorded next mechanism.
...
leak reproduction) and FREEZE_OCHO_CAPABILITY_20260907 (the working Loop: binaries, sources,
gate logs, the 16-iteration run, and the committed trajectory). Both are SHA256-verified and
mirrored. The loop client lives on branch stoch-engine-0907 (commit 257a2a1); the commit-enabled
AMD server is built from the archived pre-B3 fork tree plus the QFW_REGION_COMMIT op and the
field-identity reset.
