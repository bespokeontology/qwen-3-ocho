...
__global__ void __launch_bounds__(256) k_idx_select_rows(const float *__restrict__ idx640, const float *__restrict__ pool_key,
    ...
    __syncthreads();
    // Degenerate case first: fewer than IDX_TOPK strictly positive scores. The bisection
    // terminal state here was lo == hi == 0 with an EMPTY fill band, i.e. the selection is
    // exactly { sc > 0 } with no tie fill — reproduce that bit-exactly.
    if (tid == 0) sh_cnt = 0;
    __syncthreads();
    int posc = 0;
    ...
    // Exact single-pass top-512: 4 radix passes (most significant byte first) over the
    // order-preserving uint32 bit patterns (scores are relu sums >= 0, so __float_as_uint is
    // monotonic). th = the 512th largest score; cnt_gt = count(sc > th). Deterministic: ties
    // at th fill in ascending block order — identical to the 48-step bisection semantics.
    if (tid == 0) { sh_th = 0u; sh_above = 0; }
    __syncthreads();
    for (int pass = 0; pass < 4; pass++) {
        const int shift = (3 - pass) * 8;
        const uint32_t pref_mask = (pass == 0) ? 0u : (0xFFFFFFFFu << (shift + 8));
        const uint32_t th = sh_th;   // shared: thread 0 advances it, everyone reads it
        ...
        __syncthreads();
        if (tid == 0) {
            // the 512th of the WHOLE row is the (IDX_TOPK - sh_above)-th of this prefix
            // population: sh_above = count of keys already above the prefix so far
            int cum = 0;
            for (int v = 255; v >= 0; v--) {
                if (cum + hist[v] >= IDX_TOPK - sh_above) { sh_th |= (uint32_t)v << shift; sh_above += cum; break; }
                cum += hist[v];
            }
            // not-found is unreachable: the block total is nfull >= IDX_TOPK here
        }
        __syncthreads();
    }
...
extern "C" int qf_qsa_index_rows(float *idx640, int T, const QfDecodeParams *params, const void *idx_qnorm, const void *idx_knorm,
    ...
    int pre = 0;
    {   // scores for every row as 4 GEMMs (heads). Decode capture (T==1): pos0_host is frozen at
        // graph capture, so the GEMM/fold must cover the full allocation nb_max; the fold and the
        // selector still read only each row's device-side complete-block count (nfull_t = pos_t >> 2).
        // Prefill (T>1, eager) keeps the host-known chunk bound. g_idx_planes is sized
        // >= IDX_H*T*nb_max by the prefill path before capture (a cudaMalloc inside the captured
        // graph would break capture): at T==1 the need is 4*1*4096 floats = 64 KB.
        int nfull_max = (T == 1) ? nb_max : (int)((pos0_host + T) >> 2);
        if (nfull_max > nb_max) nfull_max = nb_max;
